    ...
    if (Platform.OS === 'android') {
      ...
      if (androidPermission === 'granted') {
        ...
        if (camResponse.didCancel) {
          Toast.show('User cancelled camera picker', Toast.LONG);
        } else if (camResponse.errorCode) {
          Toast.show(`Camera Error: ${camResponse.errorCode}`, Toast.LONG);
        } else if (camResponse.assets) {
          callBck(camResponse);
        }
      } else if (androidPermission === 'never_ask_again') {
        Toast.show('Camera permission is not granted. Please enable it in settings.', Toast.LONG);
      }
    } else {
      // launching camera
      const camResponse = await launchCamera(options);
